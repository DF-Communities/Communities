({
    formatActivities : function(component, activities) 
    {
        var helper = this;
        if (!activities) return activities;
        var _ = component.find('lodashService').get('v.instance');
        activities.forEach(function(act){
            helper.formatAttachments(act.Attachments);
        });
        return _.sortBy(activities, 'Start__c');
	},
    
    formatAttachments : function(atts) 
    {
        if (!atts) return;
        var helper = this;
        atts.forEach(function(a){
            a.length_str = helper.fileSize(a.BodyLength)
            a.downloadLink = '/services/apexrest/dfc/attachment?id='+encodeURIComponent(a.signedId);
